...
obj_loader::obj_loader(const std::string& fileName)
{
	...
	{
		fatalMeshError("Unable to Load Mesh", fileName);
	}
...
IndexedModel obj_loader::ToIndexedModel()
{
	IndexedModel result;
	...
	for (unsigned int i = 0; i < numIndices; i++)
	{
		...
		(hasUVs) ? (currentTexCoord = uvs[currentIndex->uvIndex]) : (currentTexCoord = glm::vec2(0.0f, 0.0f));

		(hasNormals) ? (currentNormal = normals[currentIndex->normalIndex]) : (currentNormal = glm::vec3(0.0f, 0.0f, 0.0f));
...
unsigned int obj_loader::FindLastVertexIndex(const std::vector<OBJIndex*>& indexLookup, const OBJIndex * currentIndex, const IndexedModel & result)
{
...
void obj_loader::CreateOBJFace(const std::string& line)
{
...
OBJIndex obj_loader::ParseOBJIndex(const std::string& token, bool* hasUVs, bool* hasNormals)
{
...
glm::vec3 obj_loader::ParseOBJVec3(const std::string& line)
{
...
glm::vec2 obj_loader::ParseOBJVec2(const std::string& line)
{
	unsigned int tokenLength = line.length();
	const char* tokenString = line.c_str();
